...
var (
	...
	QueueNameIsEmptyError = errors.New("queue name can not be empty")
	ConsumerNameIsEmptyError = errors.New("consumer name can not be empty")
...
func BytesToCommand(bytes []byte) (IMqCommand, error) {
	...
	cmdBytesPackage := utils.NewIoBuffer(bytes)
	...
	switch cmdType {
		...
		case CommandTypeProduceMessageResponse: {
			...
			msgId := utils.Combine8BytesArray2Int64(msgIdBytes)
		...
		case CommandTypeConsumeMessageWithTimeout: {
			...
			timeout := utils.Combine8BytesArray2Int64(timeoutBytes)
...
func (cmd *ProduceMessageResponse) toBytes() ([]byte, error) {
	...
	utils.SplitInt64To8BytesArrayStoreWithAlign(cmd.MsgId, msgIdBytes)
	return cmdBytes, nil
}
...
func (cmd *ConsumeMessageWithTimeoutCommand) toBytes() ([]byte, error) {
	...
	msgIdBytes := cmdBytes[1:]
	utils.SplitInt64To8BytesArrayStoreWithAlign(cmd.Timeout, msgIdBytes)
	return cmdBytes, nil
}
