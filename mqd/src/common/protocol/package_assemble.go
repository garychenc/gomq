...
type IPackageCompletedListener interface {
	OnCompleted(completedPackage *utils.IoBuffer) error
}

type PackageCompletedListenerFunc func(completedPackage *utils.IoBuffer) error

func (f PackageCompletedListenerFunc) OnCompleted(completedPackage *utils.IoBuffer) error {
	return f(completedPackage)
}

type IPackageAssemble interface {
	Assemble2CompletePackage(receivedMessage *utils.IoBuffer, completedListener IPackageCompletedListener) error
	Assemble2CompletePackageFunc(receivedMessage *utils.IoBuffer, completedListener func(completedPackage *utils.IoBuffer) error) error
}

type RpcPackageAssembleImpl struct {
	uncompletedPackages []*utils.IoBuffer
	logger              *logs.BeeLogger
}

func NewPackageAssemble(logConfig *config.LogConfig) IPackageAssemble {
	...
	uncompletedPackages := make([]*utils.IoBuffer, 0, DEFAULT_UCP_BUFFER_SIZE)
	return &RpcPackageAssembleImpl{uncompletedPackages, logger}
}

func (packageAssemble *RpcPackageAssembleImpl) Assemble2CompletePackageFunc(receivedMessage *utils.IoBuffer, completedListener func(completedPackage *utils.IoBuffer) error) error {
	return packageAssemble.Assemble2CompletePackage(receivedMessage, PackageCompletedListenerFunc(completedListener))
}

func (packageAssemble *RpcPackageAssembleImpl) Assemble2CompletePackage(receivedMessage *utils.IoBuffer, completedListener IPackageCompletedListener) error {
...
func (packageAssemble *RpcPackageAssembleImpl) mergeAndProcess(receivedMessage *utils.IoBuffer, completedListener IPackageCompletedListener) {
	mergedMessage := utils.NewIoBuffer(make([]byte, 0, utils.DEFAULT_IO_BUFFER_SIZE*2))
...
func isPackageStart(receivedMessage *utils.IoBuffer) bool {
...
func maybePackageStart(receivedMessage *utils.IoBuffer) bool {
