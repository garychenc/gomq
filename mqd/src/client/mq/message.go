...
func BytesToMessage(bytes []byte) (IMessage, error) {
	...
	msgBytesPackage := NewIoBuffer(bytes)
...
func NewBytesMessage(payload []byte, seqGen *SequenceGenerator) (*BytesMessage, error) {
	if seqGen == nil {
...
func NewEmptyBytesMessage(seqGen *SequenceGenerator) (*BytesMessage, error) {
...
func (msg *BytesMessage) toBytes() ([]byte, error) {
	...
	if msg.payload == nil {
		msgBytesPackage := NewIoBuffer(make([]byte, 0, MsgIdBytesSize + MsgPayloadFlagBytesSize))
		...
		msgBytesPackage := NewIoBuffer(make([]byte, 0, MsgIdBytesSize + MsgPayloadFlagBytesSize + MsgPayloadLengthBytesSize + msgPayloadLength))
		msgBytesPackage.WriteInt64(msg.msgId)
