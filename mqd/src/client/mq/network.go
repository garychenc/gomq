...
type LongConnNetworkClient struct {
	...
	namedRpcRequestActions IConcurrentMap
	dynamicResponseActions IConcurrentMap
	...
	packageAssembler IPackageAssemble
	packageCompletedListener IPackageCompletedListener
	receivedDataAssembleChannel chan *IoBuffer
	...
	messageIdGenerator *SequenceGenerator
...
func NewLongConnNetworkClient(clientConfig *LongConnNetworkClientConfig) INetworkClient {
	...
	networkClient.namedRpcRequestActions = NewConcurrentMapWithSize(512)
	networkClient.dynamicResponseActions = NewConcurrentMapWithSize(16 * 1024)
...
type ForWritingBizRequest struct {
	cacheLinePadding1 [CACHE_LINE_SIZE]byte
	...
	cacheLinePadding2 [CACHE_LINE_SIZE]byte
}
...
func (networkClient *LongConnNetworkClient) connectToRpcServer(appKey, appSecret, deviceId string) error {
	...
	if conn == nil {
		...
		msgIdGen, err := NewSequenceGenerator()
		if err != nil {
		...
		networkClient.packageAssembler = NewPackageAssemble(clientConfig.LogConfig)
		networkClient.packageCompletedListener = &PackageReadingCompletedListenerImpl{networkClient: networkClient}
		networkClient.receivedDataAssembleChannel = make(chan *IoBuffer, clientConfig.ReceivedDataAssembleChannelSize)
		...
		writingBuffer := NewIoBuffer(writingBufferBytes)
		err = Initialize2LongConnRegisterRequestPackage(appKey, appSecret, deviceId, "", writingBuffer)
		if err != nil {
		...
		if networkClient.registerResponseReceived {
			if networkClient.isConnRegisterSucc {
				...
				networkClient.lastHeartbeatTime = CurrentTimeMillis()
	...
	if clientConfig != nil {
		...
		if sendNextPackageWaitingTime > 0 {
			...
			if connLastSentTime > 0 {
				forWaitingTime := sendNextPackageWaitingTime - (CurrentTimeMillis() - connLastSentTime)
				if forWaitingTime > FOR_WAITING_THRESHOLD {
					time.Sleep(time.Duration(forWaitingTime) * time.Millisecond)
				}
			}

			networkClient.connLastSentTime = CurrentTimeMillis()
...
func (networkClient *LongConnNetworkClient) sendHearbeatRequestRepeatedly() {
	...
	for {
		...
		writingBufferBytes := make([]byte, 0, clientConfig.SocketWritingBufferSizeInBytes)
		writingBuffer := NewIoBuffer(writingBufferBytes)
		Initialize2LongConnHeartbeatRequestPackage(writingBuffer)
		networkClient.waitForSendNextPackage()
		...
		networkClient.heartbeatLock.Lock()
		lastHeartbeatTimeElapsed := CurrentTimeMillis() - networkClient.lastHeartbeatTime
		networkClient.heartbeatLock.Unlock()
...
func (networkClient *LongConnNetworkClient) readDataFromConnectionTask(socketConn *net.TCPConn) {
	...
	for {
		...
		if readedCount > 0 {
			...
			readedBuffer := NewIoBuffer(readedBytes)
...
func (listener *PackageReadingCompletedListenerImpl) OnCompleted(completedPackage *IoBuffer) error {
	networkClient := listener.networkClient
	decodedPackage := ValueOf(completedPackage, networkClient.connectionKey)
	functionCode := decodedPackage.FunctionCode()

	switch functionCode {

		case LC_FUNC_CODE_REGISTER_RESPONSE: {
			networkClient.logger.Info("Received a register response")
			payload := decodedPackage.Payload()
			responseCode := GetResponseCodeFromPayload(payload)
			if responseCode == REGISTER_CHECKING_RESULT_LEGAL {
				connKey := decodedPackage.ConnKey()
				sendHeartbeatInterval := GetSendHeartbeatIntervalFromPayload(payload)
				noHeartbeatResponseTimeForDeathConn := GetNoHeartbeatResponseTimeForDeathConnFromPayload(payload)
				reconnectInterval := GetReconnectIntervalFromPayload(payload)
				isCompressPushBody := GetIsCompressPushBodyFromPayload(payload)
		...
		case LC_FUNC_CODE_HEART_BEAT_RESPONSE: {
			networkClient.heartbeatLock.Lock()
			networkClient.lastHeartbeatTime = CurrentTimeMillis()
			networkClient.heartbeatLock.Unlock()
		}

		case LC_FUNC_CODE_LINE_TEST_REQUEST: {
			go networkClient.lineTestRequest(decodedPackage)
		}

		case  LC_FUNC_CODE_SERVER_2_CLIENT_CALL_RESPONSE: {
			go networkClient.client2ServerCallResponse(decodedPackage)
		}

		case  LC_FUNC_CODE_SERVER_2_CLIENT_CALL_REQUEST: {
			go networkClient.server2ClientCallRequest(decodedPackage)
		}

		case  LC_FUNC_CODE_SERVER_2_CLIENT_PUSH_REQUEST: {
...
func (networkClient *LongConnNetworkClient) lineTestRequest(lineTestRequest *WiredPackage) {
	networkClient.serviceLock.RLock()
	...
	writingBufferBytes := make([]byte, 0, clientConfig.SocketWritingBufferSizeInBytes)
	writingBuffer := NewIoBuffer(writingBufferBytes)
	Initialize2ServerLineTestResponsePackage(writingBuffer)
	networkClient.waitForSendNextPackage()
	_, err := conn.Write(writingBuffer.Bytes())
	if err != nil {
...
func (networkClient *LongConnNetworkClient) client2ServerCallResponse(rpcResponse *WiredPackage) {
	networkClient.serviceLock.RLock()
	...
	if dynamicResponseActions.Len() > 0 {
		_, messageId, actionBody, err := GetLongConnActionInfoFromPayload(rpcResponse.Payload(), isCompressPushBody)
		if err != nil {
...
func (networkClient *LongConnNetworkClient) server2ClientCallRequest(rpcRequest *WiredPackage) {
	networkClient.serviceLock.RLock()
	...
	actionName, messageId, actionBody, err := GetLongConnActionInfoFromPayload(rpcRequest.Payload(), isCompressPushBody)
	if err != nil {
		...
		if action != nil {
			if requestAction, ok := action.(IRequestAction); ok {
				...
				if err != nil {
					...
					bizRequest := &ForWritingBizRequest{funcCode: LC_FUNC_CODE_CLIENT_2_SERVER_CALL_RESPONSE,
...
func (networkClient *LongConnNetworkClient) server2ClientPushRequest(pushedRequest *WiredPackage) {
	networkClient.serviceLock.RLock()
	...
	actionName, messageId, actionBody, err := GetLongConnActionInfoFromPayload(pushedRequest.Payload(), isCompressPushBody)
	if err != nil {
	...
	if dynamicResponseActions.Len() > 0 {
		...
		if action != nil {
			if responseActionInfo, ok := action.(*DynamicResponseActionInfo); ok {
				...
				bizRequest := &ForWritingBizRequest{funcCode: LC_FUNC_CODE_CLIENT_2_SERVER_PUSH_RESPONSE,
...
func (networkClient *LongConnNetworkClient) realSendBizRequestByFuncCode(bizRequest *ForWritingBizRequest, isCompressPushBody bool,
	...
	if conn != nil {
		...
		switch bizRequest.funcCode {
			case LC_FUNC_CODE_CLIENT_2_SERVER_CALL_RESPONSE: {
				err := Initialize2ClientLongConnCallResponsePackage(actionName, messageId, payload, isCompressPushBody, writingBuffer)
				if err != nil {
			...
			case LC_FUNC_CODE_CLIENT_2_SERVER_PUSH_RESPONSE: {
				err := Initialize2ClientReliablePushResponsePackage(actionName, messageId, payload, isCompressPushBody, writingBuffer)
				if err != nil {
					networkClient.processSend2ServerError(bizRequest, bizRequestWritingRetryChannel)
			...
			case LC_FUNC_CODE_CLIENT_2_SERVER_CALL_REQUEST: {
				err := Initialize2ClientLongConnCallRequestPackage(actionName, messageId, payload, isCompressPushBody, writingBuffer)
				if err != nil {
					networkClient.processSend2ServerError(bizRequest, bizRequestWritingRetryChannel)
		...
		for _, responseActionKey := range responseActionKeys {
			...
			if responseAction != nil {
				if responseActionInfo, ok := responseAction.(*DynamicResponseActionInfo); ok {
					...
					if (CurrentTimeMillis() - actionCreatedTime) >= dynamicResponseActionRetentionTime {
...
func (networkClient *LongConnNetworkClient) RPC(rpcActionName string, rpcRequest []byte, timeout int64) (rpcResponse []byte, err error) {
	...
	responseActionInfo := &DynamicResponseActionInfo{createdTime: CurrentTimeMillis(), isWaitingForResponseReceived: true,
	...
	bizRequest := &ForWritingBizRequest{funcCode: LC_FUNC_CODE_CLIENT_2_SERVER_CALL_REQUEST, actionName: rpcActionName,
